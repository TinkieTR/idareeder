...
.setColor('#6a00ff')
.addField(`Quick | Teşekkürler`, `**Selamlar, Ben Volkan (Quick'in Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Quick | Prefix`, `**Quick Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Quick | Nasıl Kullanılır?`, `**Quick botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`__Bilgilendirme__`,`<a:syes1:751896954182697051> \`q!davet\` | Quick'i Sunucunuza Davet Edersiniz\n<a:syes1:751896954182697051> \`q!botbilgi\` | Botun İstatistiklerini Gösterir \n <a:syes1:751896954182697051> [Quick Destek Sunucusu](https://discord.gg/mVEETmhjw4)  \n <a:syes1:751896954182697051> \`q!ayarlar\` | Sunucunuzdaki Açık veya Kapalı Komutları Gösterir`)
.setFooter(`Quick | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
var oyun = [
`✨ Yardım almak için | q!yardım`,
`🚀 Yeni Özellikler İçin | q!yardım`,
`🔔 Sponsor İçin | q!sponsor`,
`⚡️ Botu eklemek için | q!davet`,
`🌟 Prefix ayarlamak için | q!prefix`
]
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!803038928281403393>`) return msg.channel.send(`> **Quick | Prefix**\n\n> <a:syes1:751896954182697051> **Sanırım beni etiketlediniz.**\n > <a:cark:724434925662503096> Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(`<a:cark:724434925662503096> <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Quick Bot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffd100")
    ...
    .setDescription(`<a:cark:724434925662503096> **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setDescription(`<a:cark:724434925662503096> ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`)
                .setFooter(`Quick Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`<a:cark:724434925662503096> ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`)
                .setFooter(`Quick Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.MessageEmbed()
                    .setColor('#ffd100')
                .setDescription(`<a:cark:724434925662503096> ${channel.name} **Adlın Metin Kanalı  Silindi**`)
                .setFooter(`Quick Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`<a:cark:724434925662503096> ${channel.name} **Adlı Ses Kanalı Silindi**`)
            .setFooter(`Quick Bot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
client.on("messageUpdate", async (oldMsg, newMsg) => {
  ...
  const embed = new Discord.MessageEmbed()
  .setColor("#ffd100")
  .addField("<a:cark:724434925662503096> Kullanıcı", oldMsg.author.tag, true)
  .addField("<a:cark:724434925662503096> Eski Mesaj",`  ${oldMsg.content}  `)
  .addField("<a:cark:724434925662503096> Yeni Mesaj", `${newMsg.content}`)
    ...
    if (küfür.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:cark:724434925662503096> **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:cark:724434925662503096> **Hey ${newMessage.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:cark:724434925662503096> **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:cark:724434925662503096> **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 's.a.' || msg.content.toLowerCase() == 'selam' || msg.content.toLowerCase() == 'slm') {
          try {
                  ...
                  return msg.reply('**<a:cark:724434925662503096> Aleyküm Selam, Hoşgeldin.** ')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:cark:724434925662503096> `"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }

  if (member.guild.memberCount == sayaç) {
    return client.channels.get(kanal).send(`<a:cark:724434925662503096> **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`)
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:cark:724434925662503096> `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
.setImage('https://media.discordapp.net/attachments/803375309193216053/803420360628109322/standard_1.gif')
    ...
    .setFooter("Quick'i Tercih Ettiğiniz İçin Teşekkür Ederiz.")
 ...
 if (mesaj) {
  ...
  const deneme = new Discord.MessageEmbed()
  .setColor('#6a00ff')
  .setDescription(mesajs)
    return client.channels.cache.get(kanal).send(deneme);
     }
})
...
client.on('message', async (msg, member, guild) => {
  let tag2 = await  db.fetch(`tag.${msg.guild.id}`)
        ...
        .setColor('#f6ff00')
        .setDescription(`<a:cark:724434925662503096> **İşte Tag :** ${tag2}`)
...
.setColor('#f6ff00')
.setDescription(`<a:cark:724434925662503096> ${newUser} **Aramıza hoşgeldin. \`${tag}\` tagını aldığı için ${role} rolü verildi!**`));
...
.setColor('#f6ff00')
         .setImage('https://media.discordapp.net/attachments/803375309193216053/803420360628109322/standard_1.gif')
.setDescription(`<a:cark:724434925662503096> ${newUser} **Aramızdan ayrıldı. \`${tag}\` tagını çıkardığı için ${role} rolü alındı!**`));
...
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)
   ...
   client.channels.cache.get('803643234005418024').send(rrrsembed);
...
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)
   ...
   client.channels.cache.get('803643215336964106').send(rrrsembed);
